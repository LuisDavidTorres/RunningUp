...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f77x - snake2.f77x;
            return i == 0 ? snake.f78y - snake2.f78y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (diffPartial != null) {
                ...
                diffPartial.f77x += range.oldListStart;
                diffPartial.f78y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (diffPartial.reverse) {
                    range2.oldListEnd = diffPartial.f77x;
                    range2.newListEnd = diffPartial.f78y;
                } else if (diffPartial.removal) {
                    range2.oldListEnd = diffPartial.f77x - 1;
                    range2.newListEnd = diffPartial.f78y;
                } else {
                    range2.oldListEnd = diffPartial.f77x;
                    range2.newListEnd = diffPartial.f78y - 1;
                }
                arrayList2.add(range2);
                if (!diffPartial.reverse) {
                    range.oldListStart = diffPartial.f77x + diffPartial.size;
                    range.newListStart = diffPartial.f78y + diffPartial.size;
                } else if (diffPartial.removal) {
                    range.oldListStart = diffPartial.f77x + diffPartial.size + 1;
                    range.newListStart = diffPartial.f78y + diffPartial.size;
                } else {
                    range.oldListStart = diffPartial.f77x + diffPartial.size;
                    range.newListStart = diffPartial.f78y + diffPartial.size + 1;
                }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake == null || snake.f77x != 0 || snake.f78y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f77x + snake.size;
                int i4 = snake.f78y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f77x + i5;
                    int i7 = snake.f78y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i6 = snake.f77x + snake.size;
                int i7 = snake.f78y + snake.size;
                int i8 = 8;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f77x + i3;
                int i5 = snake.f78y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f77x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f77x + i6, 1, this.mCallback.getChangePayload(snake.f77x + i6, snake.f78y + i6));
                    }
                }
                i = snake.f77x;
                i2 = snake.f78y;
